...
class VoteContract extends Contract {
  ...
  async createElection(
    ...
    let checkCreater = await this.checkValidCreater(ctx);
    checkCreater = JSON.parse(checkCreater);
  ...
  async createCandidate(ctx, candidateNum, electionId, profile) {
    let checkValidCreater = await this.checkValidCandidateCreater(
      ctx,
      electionId
    );
    checkValidCreater = JSON.parse(checkValidCreater);
    if (!checkValidCreater) {
      throw new Error(`not valid Creater`);
    }
  ...
  async checkValidCandidateCreater(ctx, electionId) {
  ...
  async resultHash(ctx, electionId, resultHash) {
    let election = await this.getElection(ctx, electionId);

    election = JSON.parse(election);

    if (election.result) {
      throw new Error(`Already resulted Vote`);
    }

    election.result = resultHash;

    await ctx.stub.putState(electionId, JSON.stringify(election));
  }

  async extendEndDate(ctx, electionId, newEndDate) {
    let checkValidCreater = await this.checkValidCandidateCreater(
      ctx,
      electionId
    );
